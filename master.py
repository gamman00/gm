...
def runAction(actNum):
    actNum = "/home/ubuntu/ArmPi_PC_Software/ActionGroups/" + actNum + ".d6a"
            ...
            if act is not None:
                for servo in servo_topic:
                    for i in range(0, len(act) - 2, 1):
                        if i == 3:
                            servo[i + 1].publish(roslibpy.Message({'data': math.radians(-240 * (act[2 + i] - 500)/1000.0), 'duration': act[1]}))
                        else:
                            servo[i + 1].publish(roslibpy.Message({'data': math.radians(240 * (act[2 + i] - 500)/1000.0), 'duration': act[1]}))
